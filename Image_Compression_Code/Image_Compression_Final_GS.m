...
% figure (); title('Image in YCbCr Color Space');
...
[hes,wis] = size(red);
A(:,:,1) = interp2([1:width/wis:width],[1:height/hes:height]',red,[1:width],[1:height]','linear');

[hes,wis] = size(green);
A(:,:,2) = interp2([1:width/wis:width],[1:height/hes:height]',green,[1:width],[1:height]','linear');

%interpolation for the colour image
[hes,wis] = size(bluecol);
B(:,:,3) = interp2([1:widthc/wis:widthc],[1:heightc/hes:heightc]',bluecol,[1:widthc],[1:heightc]','linear');
